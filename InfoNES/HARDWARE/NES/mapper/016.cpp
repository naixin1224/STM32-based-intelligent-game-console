...
void MAP16_MemoryWrite2(uint32_t addr, uint8_t data);
...
void MAP16_MemoryWrite(uint16_t addr, uint8_t data)
{
//  if(patch)
//  {
...
void MAP16_MemoryWrite2(uint32_t addr, uint8_t data)
{
  switch(addr & 0x000F)
  {
    ...
    case 0x000C:
      {
         MAPx->irq_latch = ((uint32_t)data << 8) | (MAPx->irq_latch & 0x00FF);
      }
      break;
