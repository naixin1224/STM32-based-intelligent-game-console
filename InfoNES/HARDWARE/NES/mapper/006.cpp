...
void MAP6_MemoryWriteLow(uint16_t addr, uint8_t data)
{
  switch(addr)
  {
    case 0x42FE:
    ...
    case 0x4502:
      {
        MAPx->irq_counter = (MAPx->irq_counter & 0xFF00) | (uint32_t)data;
      }
      break;

    case 0x4503:
      {
        MAPx->irq_counter = (MAPx->irq_counter & 0x00FF) | ((uint32_t)data << 8);
        MAPx->irq_enabled = 1;
      }
      break;
  }
...
void MAP6_MemoryWrite(uint16_t addr, uint8_t data)
{
  uint8_t prg_bank = (data & 0x3C) >> 2;
  uint8_t chr_bank = data & 0x03;
