...
//    if(patch == 2)
//    {
//      uint8_t retval =  parent_NES->ReadSaveRAM(MAPx->regs[2] & 0x7F);
...
//    }
  ...
  {
    return (uint8_t)(MAPx->irq_counter & 0x00FF);
  }
  else if ((addr & 0xF800) == 0x5800) // addr $5800-$5FFF
  {
    return (uint8_t)((MAPx->irq_counter & 0x7F00) >> 8);
  }
  else
  {
...
//          }
    ...
    case 0x5800: // addr $5000-$57FF
      {
        MAPx->irq_counter = (MAPx->irq_counter & 0x00FF) | ((uint32_t)(data & 0x7F) << 8);
...
void MAP19_MemoryWrite(uint16_t addr, uint8_t data)
{
  switch(addr & 0xF800)
  {
...
void MAP19_HSync(int scanline)
{
  if(MAPx->irq_enabled)
  {
    //if(MAPx->irq_counter >= (uint32_t)(0x7FFF - ((patch == 1) ? 112 : 113)))
    if(MAPx->irq_counter >= (uint32_t)(0x7FFF - 113))
    {
