...
void apu_process(uint16_t *buffer, int num_samples)
{
   ...
   else
   {
      ...
      while (num_samples--)
      {
         ...
         accum = 0;
         if (mix_enable[0]) accum += apu_rectangle(&apu->rectangle[0]); //矩形波
         if (mix_enable[1]) accum += apu_rectangle(&apu->rectangle[1]); //矩形波
         if (mix_enable[2]) accum += apu_triangle(&apu->triangle);      //三角波
         if (mix_enable[3]) accum += apu_noise(&apu->noise);            //噪音
         if (mix_enable[4]) accum += apu_dmc(&apu->dmc);                //dmc
...
void apu_reset(void)
{
	...
	memset(&apu->queue, 0, APUQUEUE_SIZE * sizeof(apudata_t));//"string.h"
