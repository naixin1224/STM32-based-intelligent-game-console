...
void OV2640_Start(uint8_t *color_buff,uint32_t color_buff_cnt)
{
	__HAL_DCMI_ENABLE_IT(&hdcmi,DCMI_IT_FRAME);                                            //使能帧中断
	HAL_DCMI_Start_DMA(&hdcmi,DCMI_MODE_CONTINUOUS,(uint32_t)color_buff,color_buff_cnt/4); //连续采样
}
//初始化OV2640 
...
{ 
  ...
  SCCB_Init();        		                 //初始化SCCB
	...
	OV2640_Auto_Exposure(2);     //自动曝光等级设置,level:0~4
	OV2640_Light_Mode(0);        //白平衡设置,mode:0~4
	OV2640_Color_Saturation(2);  //色度设置,sat:0~4
	OV2640_Brightness(2);        //亮度设置,bright:0~4
	OV2640_Contrast(2);          //对比度设置,contrast:0~4
	OV2640_Special_Effects(0);   //特效设置,eft:0~6
  return 0; 	
} 
//OV2640切换为JPEG模式
...
void OV2640_SetFirstByte(uint8_t first_byte)
{
	SCCB_WR_Reg(0xFF,0x00);
	SCCB_WR_Reg(0xDA,0x08 | (first_byte & 0x01)); //Bit[0]：0 RGB565先输出高字节，1 RGB565先输出低字节
}
//OV2640切换为RGB565模式
//参数first_byte：0 RGB565先输出高字节，1 RGB565先输出低字节
void OV2640_RGB565_Mode(uint8_t first_byte) 
{
	uint16_t i=0;
	//设置:RGB565输出
	...
	OV2640_SetFirstByte(first_byte);
} 
//自动曝光设置参数表,支持5个等级
const static uint8_t OV2640_AUTO_EXPOSURE_LEVEL[5][8]=
{
...
{  
	uint8_t i;
	uint8_t *p = (uint8_t*)OV2640_AUTO_EXPOSURE_LEVEL[level];
	for(i = 0; i < 4; i++)
	{ 
...
//彩条测试
//sw:0,关闭彩条
//sw:1,开启彩条(注意OV2640的彩条是叠加在图像上面的)
